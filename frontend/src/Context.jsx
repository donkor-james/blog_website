...
export const ContextProvider = ({children}) => {
    ...
    let { notifications, isConnected, setNotifications, unreadCount, setUnreadCount } = useWebSocket(
        'wss://writespace.duckdns.org/ws/general/',
    ...
    //             if (new_access) {
    //                 const retry = await fetch('https://writespace.duckdns.org/api/notification/notifications/', {
    //                     method: 'GET',
    //                     headers: {
    ...
    const refreshAccessToken = async () => {
        ...
        try {
            const response = await fetch('https://writespace.duckdns.org/api/users/token/refresh/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const fetchUser = async () => {
        ...
        try{
            ...
            const response = await fetch('https://writespace.duckdns.org/api/users/profile/', {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('access')}`,
                    'Content-Type': 'application/json'
            ...
            if (response.ok){
                ...
                const new_access = await refreshAccessToken()
                if (new_access){ 
                    const new_response = await fetch('https://writespace.duckdns.org/api/users/profile/', {
                        method: 'GET',
                        headers:{
                            'Authorization': `Bearer ${new_access}`,
                            'Content-Type': 'application/json'
